...
phase: init | review | finalize | finalized current_section: <number, only meaningful during review phase> plan_version: v1 last_updated: YYYY-MM-DD HH:MM
...
**Date:** YYYY-MM-DD **Author:** (fCMO / planner name)
...
If the user says _"start over"_ → ask whether they want to delete the existing folder or move it to `archive/` first; don't silently overwrite. If the user says _"redo Section X"_ → uncheck that box in `progress.md`, delete `sections/0X.md`, and re-draft.
